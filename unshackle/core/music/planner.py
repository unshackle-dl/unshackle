...
class MusicPlanner:
    ...
    def build(self, music: Music) -> MusicDownloadPlan:
        ...
        plan = MusicDownloadPlan(
            ...
            total_duration=getattr(music, "total_duration", None) or self.sum_duration(music),
            quality_requested=self.quality_requested(music),
        )

        selected_options: list[MusicTrackOption] = []
        discs: dict[int, list[MusicSongPlan]] = defaultdict(list)
        for song in music:
            options = self.get_options(song)
        ...
        quality = self.quality_summary_from_options(selected_options)
        if quality:
            plan.quality_requested = quality

        return plan

    def get_options(self, song: Song) -> list[MusicTrackOption]:
    ...
    @staticmethod
    def option_from_song(song: Song) -> MusicTrackOption | None:
        data = song.data if isinstance(song.data, dict) else {}
        metadata = data.get("metadata") if isinstance(data.get("metadata"), dict) else {}
        quality = MusicPlanner.first_text(data.get("quality"), metadata.get("quality"), metadata.get("quality_label"))
        duration = MusicPlanner.first_number(data.get("duration"), metadata.get("duration"))
        if not quality and duration is None:
            return None

        codec = MusicPlanner.codec_from_quality(quality)
        bit_depth, sample_rate = MusicPlanner.quality_numbers(quality)
        bitrate = MusicPlanner.bitrate_from_quality(quality)
        ...
        return MusicTrackOption(
            ...
            bitrate=bitrate,
            channels=MusicPlanner.first_number(data.get("channels"), metadata.get("channels")),
            lossless=lossless,
            hires=hires,
            atmos=atmos,
            explicit=explicit,
    ...
    def quality_requested(self, music: Music) -> str:
        ...
        return self.first_text(data.get("quality"))
    ...
    @staticmethod
    def codec_from_quality(value: str) -> str:
        lowered = value.lower()
    ...
    @staticmethod
    def quality_numbers(value: str) -> tuple[int | None, int | None]:
        bit_depth = None
        sample_rate = None
    ...
    @staticmethod
    def bitrate_from_quality(value: str) -> int | None:
    ...
    @staticmethod
    def quality_summary_from_options(options: list[MusicTrackOption]) -> str:
        if not options:
            return ""
        ...
        if "MP3" in codecs:
            return "MP3"
        return MusicPlanner.first_text(*labels)

    @staticmethod
    def sum_duration(music: Music) -> int | None:
        total = 0
        for song in music:
            data = song.data if isinstance(song.data, dict) else {}
            value = MusicPlanner.first_number(data.get("duration"))
            total += int(value or 0)
        return total or None

    @staticmethod
    def max_value(music: Music, attr: str) -> int | None:

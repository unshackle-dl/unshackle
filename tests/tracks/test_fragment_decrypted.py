...
def senc(sample_count: int = 4) -> bytes:
    return box(b"senc", b"\x00" * 4 + struct.pack(">I", sample_count) + b"\x00" * (8 * sample_count))


def piff_senc(sample_count: int = 4) -> bytes:
    body = PIFF_SENC_UUID + b"\x00" * 4 + struct.pack(">I", sample_count) + b"\x00" * (8 * sample_count)
    return box(b"uuid", body)


def moof(*children: bytes, sdi: int = 1) -> bytes:
    tfhd = box(b"tfhd", b"\x00\x00\x00\x02" + struct.pack(">II", 1, sdi))  # sample-description-index-present
    return box(b"moof", box(b"mfhd", b"\x00" * 8) + box(b"traf", tfhd + b"".join(children)))


FTYP = box(b"ftyp", b"isom\x00\x00\x02\x00isomiso2")
MOOV = box(b"moov", box(b"mvex", box(b"trex", b"\x00" * 24)))
MDAT = box(b"mdat", b"\x00" * 64)
# a compressed payload can contain anything, including plausible-looking box runs
STRAY = box(b"mdat", struct.pack(">I", 64) + b"senc" + b"\xbe\xef" * 8 + PIFF_SENC_UUID)
...
def test_all_fragments_decrypted_passes(tmp_path: Path) -> None:
    f = write(tmp_path, "clean.mp4", FTYP, MOOV, moof(), MDAT, moof(sdi=2), MDAT)
...
def test_surviving_senc_raises(tmp_path: Path) -> None:
    ...
    boxes = [FTYP, MOOV, moof(), MDAT]
    for _ in range(3):
        boxes += [moof(senc(), sdi=2), MDAT]
    f = write(tmp_path, "skipped.mp4", *boxes)
    with pytest.raises(ValueError, match=r"3/4 fragment"):
        assert_fragments_decrypted(f)


def test_surviving_cenc_senc_raises(tmp_path: Path) -> None:
    f = write(tmp_path, "cenc.mp4", FTYP, MOOV, moof(senc()), MDAT)
...
def test_leftover_piff_uuid_after_senc_removed_does_not_raise(tmp_path: Path) -> None:
    ...
    boxes = [FTYP, MOOV]
    for _ in range(3):
        boxes += [moof(piff_senc()), MDAT]
    f = write(tmp_path, "piff_leftover.mp4", *boxes)
    assert_fragments_decrypted(f)


def test_stray_senc_bytes_in_mdat_ignored(tmp_path: Path) -> None:
    f = write(tmp_path, "stray.mp4", FTYP, MOOV, moof(), STRAY, moof(sdi=2), STRAY)
    assert_fragments_decrypted(f)
...
def test_senc_text_without_plausible_size_ignored(tmp_path: Path) -> None:
    ...
    f = write(tmp_path, "text.mp4", FTYP, MOOV, moof(box(b"free", b"presence sencoded here")), MDAT)
    assert_fragments_decrypted(f)
...
def test_senc_bytes_inside_trun_payload_ignored(tmp_path: Path) -> None:
    ...
    trun = box(b"trun", b"\x00\x00\x03\x01" + struct.pack(">I", 2) + struct.pack(">I", 512) + b"senc")
    f = write(tmp_path, "trun.mp4", FTYP, MOOV, moof(trun), MDAT)
    assert_fragments_decrypted(f)
...
def test_empty_senc_on_clear_fragment_ignored(tmp_path: Path) -> None:
    ...
    f = write(tmp_path, "empty.mp4", FTYP, MOOV, moof(senc(sample_count=0)), MDAT)
    assert_fragments_decrypted(f)


def senc_override(sample_count: int) -> bytes:
    # flags&1 => AlgorithmID(3) + IV_size(1) + KID(16) precede sample_count
    head = b"\x00\x00\x00\x01" + b"\x00\x00\x08" + b"\x08" + b"\xaa" * 16
    return box(b"senc", head + struct.pack(">I", sample_count) + b"\x00" * (8 * sample_count))


def test_override_flag_shifts_sample_count_offset(tmp_path: Path) -> None:
    clear = write(tmp_path, "ovr_clear.mp4", FTYP, MOOV, moof(senc_override(0)), MDAT)
    assert_fragments_decrypted(clear)
    f = write(tmp_path, "ovr.mp4", FTYP, MOOV, moof(senc_override(5)), MDAT)
    with pytest.raises(ValueError, match=r"1/1 fragment"):
        assert_fragments_decrypted(f)
...
def test_senc_outside_traf_ignored(tmp_path: Path) -> None:
    ...
    f = write(tmp_path, "sibling.mp4", FTYP, MOOV, box(b"moof", box(b"mfhd", b"\x00" * 8) + senc()), MDAT)
    assert_fragments_decrypted(f)
...
def test_senc_nested_under_non_traf_ignored(tmp_path: Path) -> None:
    ...
    meta = box(b"meta", senc())
    f = write(tmp_path, "nested.mp4", FTYP, MOOV, box(b"moof", box(b"mfhd", b"\x00" * 8) + meta), MDAT)
    assert_fragments_decrypted(f)
...
def test_undersized_largesize_box_does_not_desync_walk(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    f = write(tmp_path, "runt.mp4", FTYP, MOOV, runt, moof(senc()), MDAT, moof(senc()), MDAT)
...
def test_largesize_and_zero_size_boxes_walked(tmp_path: Path) -> None:
    ...
    f = write(tmp_path, "large.mp4", FTYP, MOOV, frag, large, trailing)
    with pytest.raises(ValueError, match=r"1/1 fragment"):
        assert_fragments_decrypted(f)
...
def test_malformed_box_size_warns_and_does_not_raise(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    f = write(tmp_path, "overrun.mp4", FTYP, MOOV, overrun, moof(senc()), MDAT)
    with caplog.at_level("WARNING"):
        assert_fragments_decrypted(f)
    assert "malformed box size" in caplog.text
...
def test_missing_or_bogus_file_is_noop(tmp_path: Path) -> None:
    ...
    assert_fragments_decrypted(write(tmp_path, "bogus.bin", b"\x00\x01\x02\x03not-an-mp4"))
    truncated = write(tmp_path, "cut.mp4", FTYP, MOOV, moof(senc())[:20])

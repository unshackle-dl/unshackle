...
**Important:** Windscribe proxy currently only supports **US servers**. Non-US regions require Windscribe's API authentication which includes captcha solving and is not currently supported. For non-US regions, consider using NordVPN or Surfshark instead.
...
You can optionally pin specific US servers using `server_map`:
...
Once configured, use `--proxy windscribe:us` to connect through Windscribe.

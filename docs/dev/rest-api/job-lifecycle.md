...
until it writes the file. Instead it creates a **job**, hands it back to you with an
...
This page explains the states a job moves through, how the server reports live
progress, and how to cancel or retry. Clearly marked developer sections tell you how
the job worker and the manager behind the queue operate the download.
...
!!! info "Who this page is for"
    ...
    give the internal job worker and manager model, for contributors and for people
    who debug the server. You do not need them to use the API.

For the full request and response shape of each endpoint, see
[Endpoints](endpoints.md). For how the server reports errors, see
...
| Status | Meaning |
|---|---|
| `queued` | Created and waiting for a free job worker. No download has started yet. |
| `downloading` | A job worker has picked the job up and the download is in progress. |
...
job reaches one of them it never changes state again. You can remove, retry, or read
terminal jobs from history, but they will not resume.
...
!!! note "Priority does not change status"
    ...
    it will start, but it stays `queued` until a job worker picks it up. It is not
    a state change.

## Creating a job

You queue a download with `POST /api/download`. The request needs at least a `service`
...
The job starts in `queued`. The job workers auto-start on the first job you submit, so
you switch nothing else on. Keep the `job_id`. Every later call uses it.
...
!!! tip "The full parameter set"
    ...
    server-side defaults filling in anything you do not write. The
    [Endpoints](endpoints.md) page gives the complete list, their defaults, and the
    validation rules.
...
=== "All jobs (summaries)"
    ...
    You can filter the list by `status` and `service`, and order it with `sort_by`
...
!!! tip "Or subscribe instead of polling"
    ...
    Events, and closes the event stream when the job ends. See
...
results, and `progress` is `100`. This is the state you want.
...
context. When you start the server with `--debug-api`, `error_traceback` and
`worker_stderr` are also filled in, to help you find the cause.
...
The server stopped a `cancelled` job on request, before it finished. It may have
partial output, but it is not considered complete. See below.
...
!!! warning "Do not parse the body of a 204"
    ...
    not try to decode JSON from it. Examine the status code first.
...
job with the same service, title, and parameters. The retry does not change the
original job. It stays in its terminal state, and the response gives you the new job's
ID with `202`.

- You can retry only terminal jobs. A retry of a `queued` or `downloading` job returns
  `409` with error code `CONFLICT`.
- A missing job returns `404` (`JOB_NOT_FOUND`).
- The server examines the service allowlist and any download gates again for the new
  job, exactly as for the original submission. Thus the server can reject a retry with
...
line, so it will be the next one a free job worker picks up.
...
This only reorders the queue. The job's status stays `queued` until a job worker
starts it.

## History and retention

The server also writes every job that reaches a terminal state once to a persistent
history log. The job survives there even after the server removes it from the live
list.

- Read it with `GET /api/history` (newest first, with `limit` and `service`
  filters).
...
!!! note "Why the history file lives under the cache directory"
    unshackle puts `api_history.jsonl` under the cache directory on purpose, so it
    stays out of the config/data tree. It is disposable runtime state, not user configuration or
    curated data, which is exactly why clearing the cache is a safe operation against
    it.

---

## Under the hood: the job worker and manager model
...
priority deque for the jobs it bumped to the front. On the first job, it starts
a fixed pool of job worker tasks plus one cleanup task. The start is idempotent, so
later jobs reuse the same pool.

- **Concurrency**: `max_concurrent_downloads` (default **2**) caps it. That many
  jobs can be in `downloading` at once. The rest wait in `queued`.
- **Retention**: `job_retention_hours` (default **24**) controls it. An hourly
...
!!! note "Two concurrency knobs that are easy to conflate"
    `max_concurrent_downloads` caps how many separate download **jobs** operate at once.
...
!!! note "Concurrency and retention config keys"
    ...
    defaults of **2** and **24** apply. [`GET /api/config`](endpoints.md) gives the
    live values. The related `serve.history_limit` (default
    **100**) instead controls how many entries the on-disk history file keeps.

Each job worker loop drains the priority deque first, then the FIFO queue. The loop
skips the stale FIFO copy of a promoted job, so it does not do the job twice. When
a job worker takes a job it flips it to `downloading`, stamps `started_time`, does the
download, and on finish stamps `completed_time` and records history.

### Each job runs in its own subprocess

The manager does **not** operate the download in the server's event loop or even in the
server process. For each job it spawns a separate Python subprocess:

```text
python -m unshackle.core.api.download_worker <payload.json> <result.json> <progress.json>
```

Three temporary files bridge the parent and the job worker:

- **`payload.json`**: the manager writes it before launch. It contains the job ID,
...
The job worker's exit codes are `0` for success, `1` for a download error, and `2` for bad
...
job `failed` and adopts the job worker's `error_code` (or derives one by categorising the
exception).

### How progress reaches the API

While the job worker operates, the manager polls `progress.json` every **0.5 s** and copies the
fields it finds onto the live `DownloadJob`. That is what the `GET` endpoints return.

The job worker aggregates progress across all selected tracks. It weights each track's
completion fraction by its bitrate, so video and audio dominate the bar and subtitles
barely move it (subtitles get a small fixed weight). Track fractions are kept
**monotonic**: the percentage only ever climbs. The job worker scales the download
portion to fill `0` to `90`. The download engine drives the remaining `90` to `100` as
it repackages and muxes. A track counts as done when it reaches a terminal state such as
...
- If it is **`queued`**, it flips straight to `cancelled`, the manager records its
  history immediately (it never reaches a job worker), and the job worker loop skips it
  if it later pops it off the queue.
- If it is **`downloading`**, the manager sets the cancellation event and flips the job
  to `cancelled`. The polling loop notices the event, `terminate()`s the job worker
...
`token`, `api_key`) and strips userinfo from proxy URLs. The manager also scrubs
free-text fields that a service might have leaked a secret into (`error_message`,
`error_details`, `error_traceback`, `worker_stderr`, and job worker stdout/stderr in the
logs). It removes the job's known secret values, longest-match first, before it returns
or logs them.

...
| [Output & naming](#output-naming) | `output_template`, `tag`, `tag_rules`, `tag_group_name`, `tag_imdb_tmdb`, `chapter_fallback_name`, `unicode_filenames` |
...
Conditionally replaces [`tag`](#output-naming) for a given download. Each rule pairs a `when`
...
written into MKV and music files always uses [`tag`](#output-naming).
